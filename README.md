...
- I did the same for `ScoreUI.cs` and `AudioManager.cs`. This way, the Controller can trigger events knowing the View systems are there and unique, but still doesn’t need to reference them directly.
...
- If I want to change the score font or swap the flap sound later, I only need to touch the View scripts, no messing with the core game logic.
